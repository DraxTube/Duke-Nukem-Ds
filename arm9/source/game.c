...
char confilename[128] = {"/duke3d/GAME.CON"},boardfilename[128] = {0};
...
void coords(short snum)
{
    ...
    printext256(250L,y,31,-1,tempbuf,1);
...
void nonsharedkeys(void)
{
    ...
    if( KB_KeyPressed( sc_F12 ) )
    {
        ...
        dukescreencapture("duke0000.pcx",0);
...
void comlinehelp(char **argv)
{
    ...
    puts(" /xFILE        Compile FILE (default	)");
...
{
    char groupfile[] = "/duke3d/DUKE3D.GRP";
...
int main(int argc,char **argv)
{
...
#if 1
    nds_init();
    // On DSi, set ARM9 to 133MHz (default is 67MHz on DS/DSi)
    if (isDSiMode()) {
        setCpuClock(true);
    }
#else
 ...
 // 	BG3_XDX = 1 << 8;
  ...
  iprintf("\thttp://gpf.dcemu.co.uk\n\n");
	...
	//for(ka = 0; ka < 256*256; ka++)
	//	BG_GFX[ka] = ((u16*)NEO_Spring_Compo_2007_Raphael_NDS_img_bin)[ka] | BIT(15); 
	//iprintf("\tPress Any Key to continue!\n");
	//while(1){ WaitForVblank();scanKeys();if(keysDown()){ break;}}
	//lcdSwap();
...
char opendemoread(char which_demo) // 0 = mine
{
  ...
  // disable demo loading 'cos it's broken
  return 0;
