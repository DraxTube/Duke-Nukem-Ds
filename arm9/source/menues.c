...
static int loadpheader(char spot,int32 *vn,int32 *ln,int32 *psk,int32 *nump)
{
     ...
     fread((char *)waloff[MAXTILES-3],160,100,fil);
...
int loadplayer(signed char spot)
{
     ...
     if(numplayers > 1)
         ...
         fread(&ud.savegame[spot][0],19,1,fil);

     music_changed = (music_select != (ud.volume_number*11) + ud.level_number);

         fread(&ud.volume_number,4,1,fil);
         fread(&ud.level_number,4,1,fil);
         fread(&ud.player_skill,4,1,fil);
     ...
     fread(&wall[0],sizeof(walltype),MAXWALLS,fil);
         fread(&numsectors,2,1,fil);
     fread(&sector[0],sizeof(sectortype),MAXSECTORS,fil);
         fread(&sprite[0],sizeof(spritetype),MAXSPRITES,fil);
         fread(&headspritesect[0],2,MAXSECTORS+1,fil);
         fread(&prevspritesect[0],2,MAXSPRITES,fil);
         fread(&nextspritesect[0],2,MAXSPRITES,fil);
         fread(&headspritestat[0],2,MAXSTATUS+1,fil);
         fread(&prevspritestat[0],2,MAXSPRITES,fil);
         fread(&nextspritestat[0],2,MAXSPRITES,fil);
         fread(&numcyclers,sizeof(numcyclers),1,fil);
         fread(&cyclers[0][0],12,MAXCYCLERS,fil);
     fread(ps,sizeof(ps),1,fil);
     fread(po,sizeof(po),1,fil);
         fread(&numanimwalls,sizeof(numanimwalls),1,fil);
         fread(&animwall,sizeof(animwall),1,fil);
         fread(&msx[0],sizeof(long),sizeof(msx)/sizeof(long),fil);
         fread(&msy[0],sizeof(long),sizeof(msy)/sizeof(long),fil);
     fread((short *)&spriteqloc,sizeof(short),1,fil);
     fread((short *)&spriteqamount,sizeof(short),1,fil);
     fread((short *)&spriteq[0],sizeof(short),spriteqamount,fil);
         fread(&mirrorcnt,sizeof(short),1,fil);
         fread(&mirrorwall[0],sizeof(short),64,fil);
     fread(&mirrorsector[0],sizeof(short),64,fil);
     fread(&show2dsector[0],sizeof(char),MAXSECTORS>>3,fil);
     fread(&actortype[0],sizeof(char),MAXTILES,fil);
     fread(&boardfilename[0],sizeof(boardfilename),1,fil);

     fread(&numclouds,sizeof(numclouds),1,fil);
     fread(&clouds[0],sizeof(short)<<7,1,fil);
     fread(&cloudx[0],sizeof(short)<<7,1,fil);
     fread(&cloudy[0],sizeof(short)<<7,1,fil);

     fread(&scriptptrs[0],1,MAXSCRIPTSIZE,fil);
     fread(&script[0],4,MAXSCRIPTSIZE,fil);
     ...
     fread(&actorscrptr[0],4,MAXTILES,fil);
     ...
     fread(&scriptptrs[0],1,MAXSPRITES,fil);
     fread(&hittype[0],sizeof(struct weaponhit),MAXSPRITES,fil);
     ...
     for(i=0;i<MAXSPRITES;i++)
     {
        ...
        if( scriptptrs[i]&4 ) T6 += j;
         ...
         fread(&lockclock,sizeof(lockclock),1,fil);
     fread(&pskybits,sizeof(pskybits),1,fil);
     fread(&pskyoff[0],sizeof(pskyoff[0]),MAXPSKYTILES,fil);

         fread(&animatecnt,sizeof(animatecnt),1,fil);
         fread(&animatesect[0],2,MAXANIMATES,fil);
         fread(&animateptr[0],4,MAXANIMATES,fil);
     for(i = animatecnt-1;i>=0;i--) animateptr[i] = (long *)((long)animateptr[i]+(long)(&sector[0]));
         fread(&animategoal[0],4,MAXANIMATES,fil);
         fread(&animatevel[0],4,MAXANIMATES,fil);

         fread(&earthquaketime,sizeof(earthquaketime),1,fil);
     fread(&ud.from_bonus,sizeof(ud.from_bonus),1,fil);
     fread(&ud.secretlevel,sizeof(ud.secretlevel),1,fil);
     fread(&ud.respawn_monsters,sizeof(ud.respawn_monsters),1,fil);
     ud.m_respawn_monsters = ud.respawn_monsters;
     fread(&ud.respawn_items,sizeof(ud.respawn_items),1,fil);
     ud.m_respawn_items = ud.respawn_items;
     fread(&ud.respawn_inventory,sizeof(ud.respawn_inventory),1,fil);
     ud.m_respawn_inventory = ud.respawn_inventory;

     fread(&ud.god,sizeof(ud.god),1,fil);
     fread(&ud.auto_run,sizeof(ud.auto_run),1,fil);
     fread(&ud.crosshair,sizeof(ud.crosshair),1,fil);
     fread(&ud.monsters_off,sizeof(ud.monsters_off),1,fil);
     ud.m_monsters_off = ud.monsters_off;
     fread(&ud.last_level,sizeof(ud.last_level),1,fil);
     fread(&ud.eog,sizeof(ud.eog),1,fil);

     fread(&ud.coop,sizeof(ud.coop),1,fil);
     ud.m_coop = ud.coop;
     fread(&ud.marker,sizeof(ud.marker),1,fil);
     ud.m_marker = ud.marker;
     fread(&ud.ffire,sizeof(ud.ffire),1,fil);
     ud.m_ffire = ud.ffire;

     fread(&camsprite,sizeof(camsprite),1,fil);
     fread(&connecthead,sizeof(connecthead),1,fil);
     fread(connectpoint2,sizeof(connectpoint2),1,fil);
     fread(&numplayersprites,sizeof(numplayersprites),1,fil);
     fread((short *)&frags[0][0],sizeof(frags),1,fil);

     fread(&randomseed,sizeof(randomseed),1,fil);
     fread(&global_random,sizeof(global_random),1,fil);
     fread(&parallaxyscale,sizeof(parallaxyscale),1,fil);
...
//        rotatesprite(((320>>1)-(centre)-54)<<16,(y+(probey*i)-4)<<16,65536L,0,SPINNINGNUKEICON+((totalclock>>3)%7),sh,0,10,0,0,xdim-1,ydim-1);
    ...
    if( KB_KeyPressed(sc_Space) || KB_KeyPressed( sc_kpad_Enter ) || KB_KeyPressed( sc_Enter ) || (LMB && !onbar) || KB_KeyPressed(sc_LeftControl))
    {
        if(current_menu != 110)
    ...
    else if( KB_KeyPressed( sc_Escape ) || (RMB) || KB_KeyPressed(sc_LeftAlt))
    {
        onbar = 0;
...
// CTW END - REMOVED
        ...
        case 15000:
            ...
            if( KB_KeyPressed(sc_Escape) || KB_KeyPressed(sc_N) || RMB || KB_KeyPressed(sc_LeftAlt))
            {
            ...
            if(  KB_KeyPressed(sc_Space) || KB_KeyPressed(sc_Enter) || KB_KeyPressed(sc_kpad_Enter) || KB_KeyPressed(sc_Y) || LMB || KB_KeyPressed(sc_LeftControl))
            {
                KB_FlushKeyboardQueue();
                FX_StopAllSounds();
        ...
        case 1009:
	      ...
	      puts("loading");
            ...
            if( KB_KeyPressed(sc_N) || KB_KeyPressed(sc_Escape) || RMB || KB_KeyPressed(sc_LeftAlt))
            {
                KB_ClearKeyDown(sc_N);
                KB_ClearKeyDown(sc_Escape);
                KB_ClearKeyDown(sc_LeftAlt);
...
//            menutext(c,24,0,0,"CREDITS");
            ...
            if(KB_KeyPressed(sc_Escape) || KB_KeyPressed(sc_LeftAlt)) { cmenu(0); break; }
