...
#define DEFAULT_MAXRESWIDTH  dswidth
#define DEFAULT_MAXRESHEIGHT dsheight


#define UNLOCK_SURFACE_AND_RETURN  return;
...
uint16 *surface = NULL; /* This isn't static so that we can use it elsewhere AH */
static int debug_hall_of_mirrors = 0;
static Uint32 sdl_flags = 0;//SDL_HWPALETTE;
...
#ifdef __NDS__
#define sdldebug(fmt,args...) printf("BUILDDS: " fmt "\n",##args)
#else
...
static void __append_sdl_surface_flag(uint32 *_surface, char *str,
...
static void output_surface_info(uint16 *_surface)
{
...
static __inline int sdl_mouse_button_filter(uint32 const *event)
{
        /*
...
static int sdl_mouse_motion_filter(uint32 const *event)
{
    if (surface == NULL)
        return(0);
    ...
    //} /* else */
		...
		if (keysHeld() & KEY_TOUCH)
		{
			...
			//		dy=0;
      			...
      			mouse_relative_x = dx*20;
       			mouse_relative_y = dy*20;
       			mouse_x = thisXY.px*20;
       			mouse_y = thisXY.py*20;
   	...
   	if (mouse_x > dswidth) mouse_x = dswidth;
   	if (mouse_y < 0) mouse_y = 0;
   	if (mouse_y > dsheight) mouse_y = dsheight;
 ...
 */
static int attempt_fullscreen_toggle(uint16 **surface, Uint32 *flags)
{
//    long framesize = 0;
//    void *pixels = NULL;
//    Rect clip;
...
//        palette = malloc(ncolors * sizeof (palette));
//        if (palette == NULL)
//        {
//            free(pixels);
//            return(0);
...
//    output_surface_info(*surface);
     ...
     *  enter event.
     */
static __inline int handle_keypad_enter_hack(const uint16 *event)
{
...
static int sdl_key_filter(const uint16 *event)
{
	...
	//unsigned long dskey=0,relkey=0;

	////ret=read(joyfd, &key, 4); 
	//scanKeys();
	//dskey= keysDown();
	////relkey= keysUp();
	...
	//if (dskey & KEY_START)
	//{
	//	lastkey=0x1C; // Enter key
	//	keyhandler();
	//}
	//
	//if (dskey & KEY_A)
	//{
	//	lastkey=0x1D; // Control Key
	//	keyhandler();
	//}
	//
	//if (dskey & KEY_B)
	...
	//if (dskey & KEY_X)
	//{
	//	lastkey=0xE049; // PageUp Key
	//	keyhandler();
	//}
	//
	//if (dskey & KEY_Y)
	//{
	//	lastkey=0xE051; // PageDown Key
	//	keyhandler();
	//}

	//if (dskey & KEY_RIGHT)
	//{
	//	lastkey=0xE04D; // cursor right Key
	//	keyhandler();
	//}
	//	
	//if (dskey & KEY_LEFT)
	//{
	//	lastkey=0xE04B; // cursor left Key
	//	keyhandler();
	//}
	//
	//if (dskey & KEY_UP)
	//{
	//	lastkey=0xE048; // cursor up Key
	//	keyhandler();
	//}
	//
	//if (dskey & KEY_DOWN)
	//{
	//	lastkey=0xE050; // cursor down Key
	//	keyhandler();
	//}
	//
	//if (dskey & KEY_SELECT)
	//{
	//	lastkey=0x39; // spacebar Key
	//	keyhandler();
	//}

	//if (dskey & KEY_L)
	//{
	//	lastkey=0x0E; // BACKSPACE Key
	//	keyhandler();
	//}

	//if (dskey & KEY_R)
	//{
	//	lastkey=0x0F; // TAB Key
	//	keyhandler();
	//}
 ...
 //       return(0);
	...
	//if (relkey & KEY_START)
	//{
	//	lastkey += 128; // Enter key
	//	keyhandler();
	//}
	//
	//if (relkey & KEY_A)
	//{
	//	lastkey += 128; // Control Key
	//	keyhandler();
	//}
	//	
	//if (relkey & KEY_B)
	...
	//if (relkey & KEY_RIGHT)
	//{
	//	lastkey += 128; // cursor right Key
	//	keyhandler();
	//}
	//
	//if (relkey & KEY_LEFT)
	//{
	//	lastkey += 128; // cursor left Key
	//	keyhandler();
	//}
	//
	//if (relkey & KEY_UP)
	//{
	//	lastkey += 128; // cursor up Key
	//	keyhandler();
	//}
	//
	//if (relkey & KEY_DOWN)
	//{
	//	lastkey += 128; // cursor down Key
	//	keyhandler();
	//}
	//
	//if (relkey & KEY_SELECT)
	//{
	//	lastkey += 128; // spacebar Key
	//	keyhandler();
	//}
...
enum {
	DS_PRESSED,DS_RELEASED
};
//const static int DS_RELEASED=1;
//const static int DS_PRESSED=1;
static int root_sdl_event_filter(const uint16 *event)
{
    //switch (event->type)
    //{
    //    case SDL_KEYUP:
...
const static unsigned short dskeyboard_key[]= {
		0x1C, // Enter key
		0x39, // spacebar Key
		0xE04B, // cursor left Key
		0xE04D, // cursor right Key
		0xE048, // cursor up Key
		0xE050, // cursor down Key
		0x1D, // Control Key
		0x1E, // A Key
		0x0F, // TAB Key
		0x0E, // BACKSPACE Key
...
const static unsigned long ds_keyds[]= {
		KEY_START,
		KEY_SELECT,
		KEY_LEFT,
		KEY_RIGHT,
...
static void handle_events(void)
{
	...
	for(i=0;i<13;i++)
	{
		...
		if (ds_key_old_status[i]==DS_RELEASED)
		{
			if (akey&ds_keyds[i])
			{
				lastkey=dskeyboard_key[i];
				keyhandler();
				ds_key_old_status[i]=DS_PRESSED;
			}
		}
		else
		{
			if ((rkey&ds_keyds[i]))
			{
				lastkey=dskeyboard_key[i]+128;
				keyhandler();
				ds_key_old_status[i]=DS_RELEASED;
			}
		}
...
int _joystick_axis(int axis)
{
    ...
    return 0;//(SDL_JoystickGetAxis(joystick, axis));
} /* _joystick_axis */
...
int _joystick_hat(int hat)
{
    ...
    return 0;//(SDL_JoystickGetHat(joystick, hat));
} /* _joystick_axis */
...
int _joystick_button(int button)
{
    ...
    return 0;//(SDL_JoystickGetButton(joystick, button) != 0);
} /* _joystick_button */
...
{
    const char *envr = getenv(BUILD_DSDEBUG); 
...
void nds_init() {
	...
	// VRAM A (128KB) is sufficient for 256x192=49152 bytes; banks B and D no longer needed

	// Use BG_BMP8_256x256 — render directly at native DS screen resolution 256x192
	ds_bg_main = bgInit(3, BgType_Bmp8, BG_BMP8_256x256, 0, 0);
	// 1:1 scaling — no downscale matrix needed, we render at 256x192 directly
	REG_BG3PA = (1 << 8);  // 1.0 in 8.8 fixed point
	REG_BG3PB = 0;
	REG_BG3PC = 0;
	...
	/*u16* dest16;
	dest16 = (u16*)bgGetGfxPtr(ds_bg_main);
...
static void set_sdl_renderer(void)
{
	...
	//dmaCopy(splash2_bin, SUB_BG0_CR, 256*256);
    ...
    iprintf("\tDuke3DS\n");
    iprintf("\tPorted by Troy Davis(GPF)\n");
...
void getvalidvesamodes(void)
{
    ...
    //for (i = 0; (modes != (Rect **) -1) && (modes[i] != NULL); i++)
        add_vesa_mode("physical", dswidth, dsheight);
 ...
 */
{
    ...
    assert( (start + num) <= (sizeof (fmt_swap) / sizeof (plt)) );
    ...
    for (i = 0; i < num; i++)
    {
		...
		BG_PALETTE[i]=RGB15(sdlp->r>>3,sdlp->g>>3,sdlp->b>>3)|BIT(15);
    ...
    return 0;//(SDL_SetColors(surface, fmt_swap, start, num));
} /* VBE_setPalette */


int VBE_getPalette(long start, long num, char *palettebuffer)
{
    plt *sdlp;// = surface->format->palette->colors + start;
...
void _nextpage(void)
{
    ...
    if (renderer == RENDERER_SOFTWARE)
    {
		if (qsetmode == 200)
		{
			...
			DC_FlushRange((void*)frameplace, dswidth * dsheight);
			// Async DMA copy: returns immediately, CPU is free while DMA transfers
			dmaCopyWordsAsynch(3, (void*)frameplace, surface, (dswidth * dsheight + 3) >> 2);
		}
...
void drawpixels(long offset, unsigned short pixels)
{
    ...
    surface_end = (((Uint8 *) surface) + (dswidth * dsheight)) - 2;
    pos = (Uint16 *) (((Uint8 *) surface) + offset);
    if ((pos >= (Uint16 *) surface) && (pos < (Uint16 *) surface_end))
        *pos = pixels;
...
void drawpixelses(long offset, Uint32 pixelses)
{
    ...
    ;//if (SDL_MUSTLOCK(surface))
		...
		//swiWaitForVBlank();
    surface_end = (((Uint8 *)surface) + (dswidth * dsheight)) - 2;
    pos = (Uint32 *) (((Uint8 *) surface) + offset);
    if ((pos >= (Uint32 *) surface) && (pos < (Uint32 *) surface_end))
        *pos = pixelses;
...
{
    drawpixel(((long) surface + offset), drawpixel_color);
} /* drawpixel16 */
...
void fillscreen16(long offset, long color, long blocksize)
{
    ...
    surface_end = (pixels + (dswidth * dsheight)) - 1;
...
void drawline16(long XStart, long YStart, long XEnd, long YEnd, char Color)
{
    ...
    ScreenPtr = (char *) (get_framebuffer()) + XStart + (dswidth * YStart);
...
void clear2dscreen(void)
{
 ...
 //   SDL_FillRect(surface, &rect, 0);
	memset(surface,0,dswidth*dsheight);
} /* clear2dscreen */
